"""
Mean ablation of SVD directions.
...
Substitutes: per-category mean, global mean, zero, random.
...
_SUBSTITUTE_KEYS = ("cat", "global", "zero", "rand")

_DELTA_SCHEMA = {
    "_note": "* is one of: cat, global, zero, rand (ablation substitute). "
...
def _make_accumulators(n_tasks: int, vocab_size: int) -> dict:
    """Create per-substitute accumulators for ablation metrics."""
    return {
...
def run_ablation(
    ...
    *,
    ...
    For each layer, direction, and substitute (per-category mean, global mean,
    ...
    :param rand_seed: Seed for the random substitute generator
    :type rand_seed: int
    """
    ...
    for name, data in tqdm(data_categorized.items(), desc="Ablation + ANLS"):
        ...
        acc = {
            sub: _make_accumulators(n_tasks, adapter.vocab_size)
            for sub in _SUBSTITUTE_KEYS
        }
        nls_original = []
        ...
        for i in tqdm(
            ...
            for t_idx, (layer_name, dir_idx) in enumerate(tasks):
                ...
                deltas = {
                    sub: (logits_orig - logits_parts[bi]).float()
                    for bi, sub in enumerate(_SUBSTITUTE_KEYS)
                }
                preds_parts = [
                    all_preds[:actual],
        ...
        for t_idx, (ln, dir_idx) in enumerate(tasks):
            ...
            for sub in _SUBSTITUTE_KEYS:
                a = acc[sub]
                d[f"signed_mean_{sub}"] = a["delta_sum"][t_idx] / length
                d[f"abs_mean_{sub}"] = a["delta_abs_sum"][t_idx] / length
                d[f"topk_{sub}"] = torch.cat(a["topk"][t_idx], dim=0)
                d[f"botk_{sub}"] = torch.cat(a["botk"][t_idx], dim=0)
                d[f"kl_div_{sub}"] = a["kl_div"][t_idx]
                d[f"delta_gold_prob_{sub}"] = a["delta_gold_prob"][t_idx]
            layer_delta[ln][dir_idx] = d
            layer_nls[ln]["dirs"].append(dir_idx)
            for sub in _SUBSTITUTE_KEYS:
                layer_nls[ln][sub].append(sum(acc[sub]["nls"][t_idx]) / length)
        ...
        if old_delta is not None and old_summary is not None:
            for ln in old_delta:
                ...
                for i, d_idx in enumerate(old_dirs):
                    ...
                    for sub in _SUBSTITUTE_KEYS:
                        layer_nls[ln][sub].append(
                            old_summary[_ANLS_BL_KEYS[sub]][ln][i]
                        )

        for sub in _SUBSTITUTE_KEYS:
            np.save(
                cat_dir / f"nls_ablated_{sub}.npy",
                {
                    ln: [acc[sub]["nls"][t] for t, (l, _) in enumerate(tasks) if l == ln and t not in skip_tasks]
                    for ln in layer_nls
                },
                allow_pickle=True,
            )
...
def run_joint_ablation(
    ...
    *,
    ...
    extraction. Ablates all directions in a set simultaneously (multi-rank
    correction) and measures the effect.
