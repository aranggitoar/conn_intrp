...
def gap_survivors(
    ...
    all_w = torch.stack([masks[layer][c] for c in cats])
...
def direction_profile(
    ...
    weights = torch.stack([masks[layer][c] for c in cats])
    survived = weights > threshold
...
def random_baseline_masks(
    ...
    """
    Generate binary random masks matching the RNG used by
    :func:`~conn_intrp.dm.evaluate_dm_baselines`.
