...
Main Functions:
    ...
    substitute_comparison: Zero vs cat-mean vs global-mean vs random KL per category
...
def joint_kl_table(
    ...
    *,
    ...
    :param substitute: Ablation substitute (``"cat"``, ``"global"``, ``"zero"``, ``"rand"``)
    :type substitute: str
...
def cumulative_kl(
    ...
    *,
    ...
    :type category: str
    :param substitute: Ablation substitute
    :type substitute: str
...
def gold_prob_summary(
    ...
    *representation* to ``"nats"`` for raw log-prob deltas or ``"pct"``
    ...
    """
    rows = []
    key = f"delta_gold_prob_{substitute}"
...
def anls_summary(
    ...
    *,
    ...
    :type abl: dict
    :param substitute: Ablation substitute (``"cat"``, ``"zero"``, ``"global"``, ``"rand"``)
    :type substitute: str
    ...
    for cat, data in sorted(abl.items()):
        ...
        if level == "joint":
            ...
            if set_key not in sets:
                continue
            for layer in sets[set_key]:
                ablated = sets[set_key][layer].get(f"anls_{substitute}", float("nan"))
                rows.append({
                    "category": cat,
                    "layer": layer,
            ...
            abl_key = _substitute_map.get(substitute, _substitute_map["cat"])
...
def topk_botk_summary(
    ...
    *,
    ...
    """
    topk_key = f"topk_{substitute}"
    botk_key = f"botk_{substitute}"
...
def super_additivity(
    ...
    *,
    ...
    """
    kl_key = f"kl_{substitute}"
    kl_div_key = f"kl_div_{substitute}"
    set_key = f"active_{threshold}"
...
def kl_budget(
    ...
    *,
    ...
    :type threshold: float
    :param substitute: Ablation substitute for the numerator KL
    :type substitute: str
    :param budget_substitute: Which total ablation to use as budget
        denominator, ``"global"`` or ``"zero"``
    :type budget_substitute: str
    ...
    :rtype: pd.DataFrame
    """
    budget_key = f"kl_{budget_substitute}"
    rows = []
